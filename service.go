...
// Stakepool represents a bitum stakepool solely for voting delegation.
...
// Service represents a bitumweb service.
type Service struct {
	// the http client
...
// NewService creates a new bitumwebapi service.
	...
	service := Service{
		...
		Stakepools: StakepoolSet{
			"Bitum": {
				...
				Network:              "mainnet",
				URL:                  "https://pool.bitum.io",
				Launched:             getUnixTime(2019, 06, 01, 00, 00),
			},
		},
		StakepoolKeys: []string{},
	}
...
// downloadCount calculates the cummulative download count for BITUM binaries and releases
...
func insightStatus(service *Service) (map[string]interface{}, error) {
	...
	statusReq, err := http.NewRequest("GET",
		"https://explorer.bitum.io/api/status", nil)
	if err != nil {
		return nil, err
	}

	statusReq.Header.Set("User-Agent", "bitum-project/bitumweb bot")
...
// coinSupply returns the BITUM coin supply on mainnet
func coinSupply(service *Service) (map[string]interface{}, error) {
	...
	supplyReq, err := http.NewRequest("GET", "https://explorer.bitum.io/api/status?q=getCoinSupply", nil)
	if err != nil {
		return nil, err
	}

	supplyReq.Header.Set("User-Agent", "bitum-project/bitumweb bot")
...
func stakepoolStats(service *Service, key string, apiVersion int) error {
	...
	poolReq.Header.Set("User-Agent", "bitum-project/bitumweb bot")
...
// stakepoolData fetches statistics for all listed BITUM stakepools
...
// It returns statistics on the BITUM blockchain and the available coin supply
